...
int main()
{
	...
	world = new char* [worldMaxRow]; // array of pointers
	screen = new char* [worldMaxRow];
	for (int i = 0; i < worldMaxRow; i++)
	{
		world[i] = new char[worldMaxCol]; // each pointer points to an array
	...
	while (true) // yes, its infinite! (Ctrl-C to break from program)
	{
		...
		if (movement == 'w')
		{
			potentialLocation = max(0, playerRow - 1);
			if (world[potentialLocation][playerCol] != wall)
			{
				playerRow = potentialLocation;
			}
		}
		if (movement == 's')
		{
			potentialLocation = min(worldMaxRow - 1, playerRow + 1);
			if (world[potentialLocation][playerCol] != wall)
			{
				playerRow = potentialLocation;
			}
		...
		if (movement == 'a') 
		{
			potentialLocation = max(0, playerCol - 1);
			if (world[playerRow][potentialLocation] != wall)
			{
				playerCol = potentialLocation;
			}
		}

		if (movement == 'd')
		{
			potentialLocation = min(worldMaxCol - 1, playerCol + 1);
			if (world[playerRow][potentialLocation] != wall)
			{
				playerCol = potentialLocation;
			}
