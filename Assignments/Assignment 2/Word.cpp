...
Word::Word(const string& n, const string& t, const string& d) : name(n), type(t), definition(d)
{
}
...
void Word::printDefinition() const
{
  ...
  if (type == "n")
    ...
    fullType = "(preposition)";
  else if (type == "pn")
    fullType = "(proper noun)";
  else if (type == "n_and_v")
    fullType = "(noun and verb)";
  else if (type == "misc")
    fullType = "(misc)";
...
bool Word::includesLetters(const string& yellow) const
{
  ...
  for (size_t i = 0; i < yellow.length(); i++)
  {
